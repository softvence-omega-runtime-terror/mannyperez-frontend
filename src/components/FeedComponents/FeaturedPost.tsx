...
const FeaturedPost: React.FC<FeaturedPostProps> = ({ onBuyNow }) => {
  ...
  const handleLike = (productId: string) => {
    setLikesMap((prev) => ({
      ...
      [productId]:
        (prev[productId] ||
          products.find((p: any) => p._id === productId)?.socialDetails
            ?.likes ||
          0) + 1,
    }));
  };
  ...
  const handleAddComment = (productId: string) => {
    ...
    setCommentsMap((prev) => ({
      ...
      [productId]: [
        ...(prev[productId] ||
          products.find((p: { _id: string }) => p._id === productId)
            ?.socialDetails?.comments ||
          []),
        newComment,
      ],
    }));
          ...
          >
            ...
            <div className="flex items-center justify-between py-2 border-t border-gray-100">
              ...
              <div className="flex items-center space-x-3">
                ...
                <button
                  onClick={() =>
                    sellerId &&
                    navigate(`/feed/messages/${sellerId}/${product._id}`)
                  }
            ...
            <div className="flex items-center space-x-3 pt-4 border-t border-gray-100">
              ...
              <input
                ...
                onKeyDown={(e) =>
                  e.key === "Enter" && handleAddComment(product._id)
                }
              />
            ...
            <div className="mt-4 max-h-48 overflow-y-auto space-y-2 border-t border-gray-100 pt-2">
              {comments.map((comment) => (
                <div
                  key={comment.id}
                  className="flex items-start space-x-3 py-2 border-b border-gray-50 last:border-b-0"
                >
